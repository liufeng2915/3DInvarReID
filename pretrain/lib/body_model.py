...
class BaseModel(nn.Module):
    ...
    def prepare_cond(self, batch):
        ...
        cond['betas'] = batch['z_naked_shape']

        z_naked_shape = batch['z_naked_shape']
        cond['latent'] = self.generator(z_naked_shape) #[1,64,16,64,64]
        cond['lbs'] = z_naked_shape
    ...
    def training_step_single(self, current_epoch, batch):
        ...
        reg_naked_shape = torch.mean(-0.5 * torch.sum(1 + batch['z_naked_shape_var'] - batch['z_naked_shape_mean'] ** 2 - batch['z_naked_shape_var'].exp(), dim = 1), dim = 0)
        reg_clothed_shape = torch.mean(-0.5 * torch.sum(1 + batch['z_clothed_shape_var'] - batch['z_clothed_shape_mean'] ** 2 - batch['z_clothed_shape_var'].exp(), dim = 1), dim = 0)
        ...
        if current_epoch < self.opt.nepochs_pretrain:
            ...
            if self.opt.lambda_bone_occ > 0:
                ...
                if self.opt.pretrain_bone:
                    ...
                    loss_bone_occ_clothed = F.binary_cross_entropy_with_logits(outputs['occ_clothed'], occ_gt.unsqueeze(-1))
                else:
                    outputs = self.forward(batch['smpl_verts_cano'], None, cond, canonical=True)
                    loss_bone_occ = F.binary_cross_entropy(outputs['occ'], torch.ones_like(outputs['occ']).type_as(outputs['occ']))
                    loss_bone_occ_clothed = F.binary_cross_entropy(outputs['occ_clothed'],  torch.ones_like(outputs['occ_clothed']).type_as(outputs['occ']))
            ...
            if self.opt.lambda_bone_w > 0:
                ...
                if self.opt.pretrain_bone:
                    ...
                    w_pd_cloth = self.deformer.query_weights(pts_c, {'latent': cond['lbs_cloth'], 'betas': cond['betas'] * 0})
                    loss_bone_w = F.mse_loss(w_pd, w_gt)
                    loss_bone_w_cloth = F.mse_loss(w_pd_cloth, w_gt)
                else:
                    w_pd = self.deformer.query_weights(batch['smpl_verts_cano'], {'latent': cond['lbs'], 'betas': cond['betas'] * 0})
                    w_pd_cloth = self.deformer.query_weights(batch['smpl_verts_cano'], {'latent': cond['lbs_cloth'], 'betas': cond['betas'] * 0})
        ...
        pts_c_cloth = self.deformer.query_cano(batch['smpl_verts_cano'], {'betas': cond['betas']})
        ...
        # Latent loss
        loss_latent_class = F.cross_entropy(self.fc_class_id(batch['z_naked_shape_mean']), batch['id_index']) + F.cross_entropy(self.fc_class_cloth(batch['z_clothed_shape_mean']), batch['cloth_index'])
    ...
    def training_step(self, current_epoch, batch):
        ...
        batch['z_naked_shape'] = self.reparameterize(self.z_naked_shapes_mean(batch['index']), self.z_naked_shapes_var(batch['index']))
        batch['z_clothed_shape'] = self.reparameterize(self.z_clothed_shapes_mean(batch['index']), self.z_clothed_shapes_var(batch['index']))
        batch['z_naked_shape_mean'], batch['z_naked_shape_var'] = self.z_naked_shapes_mean(batch['index']), self.z_naked_shapes_var(batch['index'])
        batch['z_clothed_shape_mean'], batch['z_clothed_shape_var'] = self.z_clothed_shapes_mean(batch['index']), self.z_clothed_shapes_var(batch['index'])

        wandb.log({"z_naked_shape": wandb.Histogram(np_histogram=np.histogram(batch['z_naked_shape_mean'].data.cpu().numpy()))})
        wandb.log({"z_clothed_shape": wandb.Histogram(np_histogram=np.histogram(batch['z_clothed_shape_mean'].data.cpu().numpy()))})
    ...
    def extract_mesh(self, current_epoch, smpl_verts, smpl_tfs, cond, res_up=3):
        ...
        mesh1, mesh2 = generate_mesh(occ_func, current_epoch, self.opt.nepochs_pretrain_coarse, smpl_verts.squeeze(0),res_up=res_up)
        ...
        mesh1['weights_color'] = torch.tensor(weights2colors(mesh1['weights'].data.cpu().numpy()), device=smpl_verts.device).float().clamp(0,1)
        mesh1['pts_c'] = outputs1['pts_c'][0].detach()
        mesh2['weights'] = outputs2['weights'][0].detach()#.clamp(0,1)[0]
        mesh2['weights_color'] = torch.tensor(weights2colors(mesh2['weights'].data.cpu().numpy()), device=smpl_verts.device).float().clamp(0,1)
    ...
    def plot(self, current_epoch, batch):
        ...
        surf_pred_cano, surf_pred_cano_clothed = self.extract_mesh(current_epoch, batch['smpl_verts_cano'], batch['smpl_tfs'], cond, res_up=3)
