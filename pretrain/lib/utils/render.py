...
class Renderer():
    ...
    def render_mesh_dict(self, mesh_dict, mode='npat'):
        ...
        with torch.no_grad():
            ...
            if 'w' in mode:
                assert('weights_color' in mesh_dict)
                mesh_teture = Meshes(mesh_dict['verts'][None], mesh_dict['faces'][None], textures=Textures(verts_rgb=mesh_dict['weights_color'][None]*shades))
