...
class ImplicitNetwork(nn.Module):
    def __init__(
        ...
        self.final_acti_type = final_acti_type
    ...
    def forward(self, input, cond, input_feat=None,  mask=None, return_feat=False, spatial_feat=False, val_pad=0, normalize=False):
        ...
        #if return_feat:
        #    feat, x = torch.split(x, [x.shape[-1] - 1, 1], dim=-1)
        ...
        if input_dim == 3:
            ...
            #feat_full = torch.ones((n_batch, n_point, feat.shape[-1]), device=x.device)
            #feat_full[mask] = feat
            #feat = feat_full


            if return_feat:
                #feat_full = torch.ones( (n_batch, n_point, x.shape[-1]-1), device=x.device) * val_pad
                #feat_full[mask] = 0
                #feat = feat_full
                feat, x = torch.split(x, [x.shape[-1]-1, 1], dim=-1)

        if self.final_acti_type == 'sigmoid':
            x = self.sigmoid(x)
        elif self.final_acti_type == 'tanh':
            x = self.tanh(x)
...
class TextureNetwork(nn.Module):
    def __init__(
        ...
        for l in range(0, self.num_layers - 1):
            ...
            setattr(self, "lin" + str(l), lin)
            setattr(self, "bn" + str(l), bn)

        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()

    def forward(self, input, cond, input_feat=None, mask=None, spatial_feat=False, val_pad=0):
